...
litはFUSEベースでディレクトリ全体をマウントし、編集操作をCRDTログとして収集する「git風ファイルシステム型」ソースコード管理ツールです。通常の`git add`ではなく`lit track`/`lit untrack`で管理対象を指定し、`lit on`でディレクトリをマウントすると以後の編集が自動的に追跡されます。
